	...
	while( (ent=readdir(pDir))!=NULL )
	{
		...
		if( S_ISDIR(statbuf.st_mode) )  //is a dir
		{
			if(strcmp( ".",ent->d_name) == 0 || strcmp( "..",ent->d_name) == 0) 
				continue;
			memset(fw_path,0,sizeof(fw_path));
			sprintf(fw_path,"%s/$$test_mode$$.bin",dir);
			if( (fw_fp=fopen(fw_path,"rb")) != NULL )
			{
				hasFW=TRUE;
				strcpy(path,fw_path);
...
int main(int argc, char const *argv[])
{
	...
	system("ifconfig eth0 192.168.222.254 up");
	system("mkdir /tmp/dnsmasq.d");
	system("dnsmasq -C /etc/dnsmasq/dnsmasq.conf -p 53 -k &");
	...
	system("echo 500 > /sys/class/leds/system\:led\:blue/delay_on ");		
	system("echo 500 > /sys/class/leds/system\:led\:blue/delay_off ");
